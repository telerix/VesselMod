...
import vesselmod.powers.VoidUnityPower;
import vesselmod.util.CardInfo;

import static vesselmod.VesselMod.makeID;

public class VoidUnity extends BaseCard {
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        ...
        for (AbstractPower pow : p.powers) {
            if (pow.ID.equals(VoidUnityPower.POWER_ID)) {
                powerExists = true;
                break;
            }
        }
        if (!powerExists) {
            this.addToBot(new ApplyPowerAction(p, p, new VoidUnityPower(p)));
        }
    }

    @Override
    public AbstractCard makeCopy(){
        return new VoidUnity();
    }
}
